...
def find_project_root() -> Path:
    """Findet den Z-Panel-Projektordner unabhängig vom Ablageort des Skripts."""
    script_dir = Path(__file__).resolve().parent
    candidates = [script_dir, *script_dir.parents]

    for candidate in candidates:
        if (candidate / "newsredaktion").is_dir() and (candidate / "assets").is_dir():
            return candidate

    # Rückwärtskompatibilität zur bisherigen Ordnerstruktur:
    # Skript lag häufig in einem Unterordner wie generator/.
...
RESEARCH_SOURCES = [
    ("Zustand", "PIK Potsdam", "https://www.pik-potsdam.de/de/aktuelles/nachrichten"),
    ("Zustand", "Copernicus Climate", "https://climate.copernicus.eu/news"),
    ("Zustand", "Copernicus Marine", "https://marine.copernicus.eu/news"),
    ("Zustand", "WMO", "https://wmo.int/news"),
    ("Zustand", "UNEP", "https://www.unep.org/news-and-stories"),
    ("Zustand", "Stockholm Resilience Centre", "https://www.stockholmresilience.org/research/research-news.html"),
    ("Zustand", "AWI", "https://www.awi.de/ueber-uns/service/presse.html"),
    ("Zustand", "GEOMAR", "https://www.geomar.de/news"),
    ("Zustand", "Umweltbundesamt", "https://www.umweltbundesamt.de/presse"),
    ("Zustand", "Bundesamt für Naturschutz", "https://www.bfn.de/aktuelles"),
    ("Zustand", "Thünen-Institut", "https://www.thuenen.de/de/newsroom"),
    ("Lösung", "European Environment Agency", "https://www.eea.europa.eu/en/newsroom"),
    ("Lösung", "Our World in Data", "https://ourworldindata.org/latest"),
    ("Querschnitt", "Deutsches Zentrum für Integrations- und Migrationsforschung", "https://www.dezim-institut.de/presse/presse-detail/"),
    ("Querschnitt", "Bundeszentrale für politische Bildung", "https://www.bpb.de/kurz-knapp/taegliche-dosis-politik/"),
    ("Querschnitt", "World Happiness Report", "https://www.worldhappiness.report/news"),
]

RESEARCH_TYPES = ("Zustand", "Lösung", "Querschnitt")

RESEARCH_PERIODS = {
    "Seit gestern": "seit gestern",
    "Letzte Woche": "aus den letzten sieben Tagen",
    "Letzter Monat": "aus den letzten 30 Tagen",
    "Letztes Jahr": "aus den letzten zwölf Monaten",
}

RESEARCH_PROFILES = {
    "Daily – Infoscreen": ("Seit gestern", 10),
    "Weekly – Wochenrückblick": ("Letzte Woche", 15),
    "Monthly – Monatsrückblick": ("Letzter Monat", 20),
    "Yearly – Jahresrückblick": ("Letztes Jahr", 30),
}
...
class NewsStudio(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("ZUSTAND News Studio 3.0")
    ...
    def _build(self):
        ...
        header.pack(fill="x")
        ttk.Label(
            header, text="ZUSTAND News Studio 3.0",
            font=("Segoe UI", 20, "bold")
        ).pack(anchor="w")
        ttk.Label(
            header,
            text="Recherche, Kurzmeldungen, Quellenlinks und Titelbilder für den Infoscreen"
        ).pack(anchor="w")
        ...
        self.research_tab = ttk.Frame(self.tabs)
        ...
        self.tabs.add(self.research_tab, text="Recherche")
        self.tabs.add(self.article_tab, text="Meldungen")
    ...
    def _build_overview(self):
        ...
        ttk.Button(
            actions, text="Recherche-Assistent",
            command=lambda: self.tabs.select(self.research_tab)
        ).pack(side="left")
        ttk.Button(
            actions, text="Neue Meldung",
        ...
        text = (
            "1. Im Recherche-Assistenten auf „Recherche starten“ klicken.\n"
            "2. Eine relevante Meldung auswählen und in den Meldungseditor übernehmen.\n"
            "3. Kurztext und Quellenlink prüfen, anschließend ein Titelbild ergänzen.\n"
    ...
    def _build_research(self):
        outer = ttk.Frame(self.research_tab, padding=14)
        outer.pack(fill="both", expand=True)
        ...
        ttk.Label(
            header,
            text="Recherche-Assistent",
            font=("Segoe UI", 16, "bold"),
        ).pack(side="left")
        ttk.Button(
            header,
            text="Recherche starten",
            command=self.start_research_assistant,
        ).pack(side="right")
        ttk.Button(
            header,
            text="Prompt nur kopieren",
            command=self.copy_morning_research_prompt,
        ).pack(side="right", padx=(0, 8))

        ttk.Label(
            outer,
            text=(
                "Zeitraum und Profil wählen, Recherche in ChatGPT starten und anschließend "
                "die gesamte JSON-Antwort mit einem Klick importieren. Die Recherche läuft "
                "weiterhin ohne API-Schlüssel über dein normales ChatGPT."
            ),
            wraplength=1000,
            justify="left",
        ).pack(anchor="w", pady=(4, 8))

        settings = ttk.LabelFrame(outer, text="Recherche-Einstellungen", padding=10)
        settings.pack(fill="x", pady=(0, 10))

        self.research_profile = tk.StringVar(value="Daily – Infoscreen")
        self.research_period = tk.StringVar(value="Seit gestern")
        self.research_max_results = tk.StringVar(value="10")

        ttk.Label(settings, text="Profil").grid(row=0, column=0, sticky="w")
        profile_box = ttk.Combobox(
            settings,
            textvariable=self.research_profile,
            values=tuple(RESEARCH_PROFILES),
            state="readonly",
            width=28,
        )
        profile_box.grid(row=0, column=1, padx=(6, 18), sticky="w")
        profile_box.bind("<<ComboboxSelected>>", self.apply_research_profile)

        ttk.Label(settings, text="Zeitraum").grid(row=0, column=2, sticky="w")
        ttk.Combobox(
            settings,
            textvariable=self.research_period,
            values=tuple(RESEARCH_PERIODS),
            state="readonly",
            width=18,
        ).grid(row=0, column=3, padx=(6, 18), sticky="w")

        ttk.Label(settings, text="Max. Meldungen").grid(row=0, column=4, sticky="w")
        ttk.Combobox(
            settings,
            textvariable=self.research_max_results,
        ...
        ttk.Button(
            settings,
            text="ChatGPT-Ergebnis importieren",
            command=self.open_research_import_dialog,
        ...
        left = ttk.LabelFrame(paned, text="Quellen", padding=10)
        right = ttk.LabelFrame(paned, text="Fund übernehmen", padding=12)
        paned.add(left, weight=2)
        paned.add(right, weight=3)

        filter_row = ttk.Frame(left)
        filter_row.pack(fill="x", pady=(0, 8))
        ttk.Label(filter_row, text="Themenart").pack(side="left")
        self.research_type_filter = tk.StringVar(value="Alle")
        ttk.Combobox(
            filter_row,
            textvariable=self.research_type_filter,
            values=("Alle",) + RESEARCH_TYPES,
            state="readonly",
            width=16,
        ).pack(side="left", padx=8)
        ttk.Button(
            filter_row,
            text="Filtern",
            command=self.refresh_research_sources,
        ).pack(side="left")

        self.research_source_tree = ttk.Treeview(
            left,
            columns=("type", "source"),
            show="headings",
            selectmode="browse",
        ...
        self.research_source_tree.heading("type", text="Art")
        self.research_source_tree.heading("source", text="Quelle")
        self.research_source_tree.column("type", width=95, anchor="w")
        self.research_source_tree.column("source", width=260, anchor="w")
        self.research_source_tree.pack(fill="both", expand=True)
        self.research_source_tree.bind("<Double-1>", lambda _e: self.open_selected_research_source())
        ...
        ttk.Button(
            source_buttons,
            text="Quelle öffnen",
            command=self.open_selected_research_source,
        ).pack(side="left")
        ttk.Button(
            source_buttons,
            text="Alle Quellen dieser Art öffnen",
            command=self.open_filtered_research_sources,
        ).pack(side="left", padx=8)

        self.research_vars = {
            "type": tk.StringVar(value="Zustand"),
            "priority": tk.StringVar(value="4"),
            "title": tk.StringVar(),
            "boundary": tk.StringVar(),
            "keywords": tk.StringVar(),
            "url": tk.StringVar(),
        }

        row = ttk.Frame(right)
        ttk.Label(row, text="Meldungsart", width=18).pack(side="left")
        ttk.Combobox(
            row,
            textvariable=self.research_vars["type"],
            values=RESEARCH_TYPES,
            state="readonly",
        ).pack(side="left", fill="x", expand=True)
        row.pack(fill="x", pady=3)

        row = ttk.Frame(right)
        ttk.Label(row, text="Priorität (1–5)", width=18).pack(side="left")
        ttk.Combobox(
            row,
            textvariable=self.research_vars["priority"],
            values=("1", "2", "3", "4", "5"),
            state="readonly",
        ).pack(side="left", fill="x", expand=True)
        row.pack(fill="x", pady=3)

        FieldRow(right, "Titel", self.research_vars["title"])

        ttk.Label(right, text="Kurztext für den Infoscreen").pack(anchor="w", pady=(10, 3))
        self.research_summary = tk.Text(right, height=8, wrap="word")
        self.research_summary.pack(fill="x")

        row = ttk.Frame(right)
        ttk.Label(row, text="Planetare Grenze", width=18).pack(side="left")
        ttk.Combobox(
            row,
            textvariable=self.research_vars["boundary"],
            values=BOUNDARIES,
        ).pack(side="left", fill="x", expand=True)
        row.pack(fill="x", pady=3)

        FieldRow(right, "Schlagwörter", self.research_vars["keywords"])
        FieldRow(right, "Quellenlink", self.research_vars["url"])

        ttk.Label(
            right,
            text=(
                "Kurztext-Regel: sachlich, verständlich, möglichst 350–550 Zeichen; "
                "klar benennen, was neu ist und warum es für die Lebensgrundlagen "
                "oder ein Querschnittsthema relevant ist."
            ),
            wraplength=560,
            justify="left",
        ...
        ttk.Button(
            buttons,
            text="In Meldungseditor übernehmen",
            command=self.transfer_research_to_article,
        ).pack(side="left")
        ttk.Button(
            buttons,
            text="Felder leeren",
            command=self.clear_research_form,
        ).pack(side="left", padx=8)

        self.refresh_research_sources()

    def refresh_research_sources(self):
        for item in self.research_source_tree.get_children():
            self.research_source_tree.delete(item)
        selected_type = self.research_type_filter.get()
        for index, (kind, name, url) in enumerate(RESEARCH_SOURCES):
            if selected_type != "Alle" and kind != selected_type:
                continue
            self.research_source_tree.insert(
                "", "end", iid=str(index), values=(kind, name), tags=(url,)
            )

    def _selected_research_source(self):
        selection = self.research_source_tree.selection()
        if not selection:
            messagebox.showinfo("Quelle auswählen", "Bitte zuerst eine Quelle auswählen.")
            return None
        index = int(selection[0])
        return RESEARCH_SOURCES[index]

    def open_selected_research_source(self):
        source = self._selected_research_source()
        if not source:
            return
        kind, name, url = source
        webbrowser.open(url)
        self.research_vars["type"].set(kind)
        self.status_var.set(f"Quelle geöffnet: {name}")

    def open_filtered_research_sources(self):
        selected_type = self.research_type_filter.get()
        sources = [
            item for item in RESEARCH_SOURCES
            if selected_type == "Alle" or item[0] == selected_type
        ]
        if len(sources) > 8:
            proceed = messagebox.askyesno(
                "Viele Browser-Tabs",
                f"Es würden {len(sources)} Quellen geöffnet. Fortfahren?"
            )
            if not proceed:
                return
    ...
    def clear_research_form(self):
        for key, var in self.research_vars.items():
            if key == "type":
                var.set("Zustand")
            elif key == "priority":
                var.set("4")
                ...
                var.set("")
        self.research_summary.delete("1.0", "end")

    def transfer_research_to_article(self):
        title = self.research_vars["title"].get().strip()
        summary = self.research_summary.get("1.0", "end").strip()
        url = self.research_vars["url"].get().strip()
        if not title or not summary:
            messagebox.showwarning(
                "Angaben fehlen",
                "Bitte mindestens Titel und Kurztext eintragen."
            )
            return
        if not valid_url(url):
            messagebox.showwarning(
                "Quellenlink prüfen",
                "Bitte einen vollständigen Quellenlink mit http:// oder https:// eintragen."
            )
            return

        self.new_article()
        self.article_vars["title"].set(title)
        self.article_vars["planetaryBoundary"].set(
            self.research_vars["boundary"].get().strip()
        )
        keywords = self.research_vars["keywords"].get().strip()
        kind = self.research_vars["type"].get().strip()
        priority = self.research_vars["priority"].get().strip()
        extra = [x for x in (keywords, kind, f"Priorität {priority}") if x]
        self.article_vars["keywords"].set(", ".join(extra))
        self.article_vars["sourceUrl"].set(url)
        self.summary_text.delete("1.0", "end")
        self.summary_text.insert("1.0", summary)
        self.tabs.select(self.article_tab)
        self.status_var.set("Recherchefund wurde in eine neue Meldung übernommen.")

    def apply_research_profile(self, _event=None):
        profile = RESEARCH_PROFILES.get(self.research_profile.get())
        if not profile:
            return
        period, maximum = profile
        self.research_period.set(period)
        self.research_max_results.set(str(maximum))

    def start_research_assistant(self):
        prompt = self._morning_research_prompt()
        ...
        try:
            ...
            messagebox.showerror("Browser konnte nicht geöffnet werden", str(exc))
            return

        messagebox.showinfo(
            "Recherche-Assistent gestartet",
            (
                "ChatGPT wurde geöffnet und der Rechercheauftrag liegt in der "
                "Zwischenablage.\n\n"
                "1. Im ChatGPT-Eingabefeld Strg+V drücken.\n"
                "2. Nachricht absenden.\n"
                "3. Die vollständige JSON-Antwort kopieren.\n"
                "4. Im News Studio auf „ChatGPT-Ergebnis importieren“ klicken."
            ),
        )
        self.status_var.set("Recherche gestartet; Prompt ist kopiert.")

    def _morning_research_prompt(self) -> str:
        source_names = ", ".join(name for _, name, _ in RESEARCH_SOURCES)
        period = RESEARCH_PERIODS.get(
            self.research_period.get(), "seit gestern"
        )
        try:
            maximum = int(self.research_max_results.get())
        except ValueError:
            maximum = 10

        return (
            f"Recherchiere die wichtigsten neuen Meldungen {period} aus seriösen "
            ...
            f"Schlage höchstens {maximum} Meldungen vor. Achte auf eine ausgewogene "
            "Mischung, darunter mindestens zwei positive oder konstruktive Meldungen, "
            "sofern seriös belegbare vorhanden sind. Prüfe besonders diese Quellen: "
            f"{source_names}. "
            "WICHTIG: Gib ausschließlich ein gültiges JSON-Array aus, ohne Markdown, "
            "ohne Einleitung und ohne Erläuterung. Jeder Eintrag muss exakt diese "
            "Felder enthalten: "
            '{"priority":5,"type":"Zustand|Lösung|Querschnitt",'
            '"title":"...","summary":"350–550 Zeichen",'
            '"boundary":"planetare Grenze oder Querschnittsthema",'
            '"keywords":["..."],"sourceTitle":"Titel der Originalquelle",'
            '"sourceUrl":"https://direkter-link-zur-originalquelle",'
            '"publicationDate":"YYYY-MM-DD"}. '
            "Nutze nur direkte, geprüfte Originalquellenlinks. Wenn kein gültiger "
            "Direktlink feststellbar ist, lasse die Meldung weg."
        )

    def copy_morning_research_prompt(self):
        prompt = self._morning_research_prompt()
        self.clipboard_clear()
        self.clipboard_append(prompt)
        self.update()
        messagebox.showinfo(
            "Rechercheauftrag kopiert",
            "Der Rechercheauftrag wurde in die Zwischenablage kopiert."
        )

    def open_research_import_dialog(self):
        dialog = tk.Toplevel(self)
        dialog.title("ChatGPT-Ergebnis importieren")
        dialog.geometry("900x650")
        dialog.transient(self)

        ttk.Label(
            dialog,
            text="Vollständige JSON-Antwort aus ChatGPT einfügen:",
            font=("Segoe UI", 12, "bold"),
        ).pack(anchor="w", padx=14, pady=(14, 6))

        editor = tk.Text(dialog, wrap="none")
        ...
        ttk.Button(
            buttons,
            text="JSON prüfen und importieren",
            command=lambda: self.import_research_json(
                editor.get("1.0", "end").strip(), dialog
            ),
        ).pack(side="left")
        ttk.Button(
            buttons, text="Abbrechen", command=dialog.destroy
        ).pack(side="right")
    ...
    def import_research_json(self, raw_text, dialog=None):
        ...
        for folder in (ENTWUERFE, ARTIKEL):
            for path in json_files(folder):
                try:
                    existing_ids.add(str(read_json(path).get("id", "")))
                except Exception:
                    pass

        for index, item in enumerate(items, start=1):
            if not isinstance(item, dict):
                skipped.append(f"Meldung {index}: kein Objekt")
                continue

            title = str(item.get("title", "")).strip()
            summary = str(item.get("summary", "")).strip()
            source_url = str(item.get("sourceUrl", "")).strip()
            if not title or not summary or not valid_url(source_url) or not source_url:
                skipped.append(
                    f"Meldung {index}: Titel, Kurztext oder gültiger Quellenlink fehlt"
                )
                continue

            base_id = datetime.now().strftime("%Y%m%d") + "_" + slug(title)[:24]
            article_id = base_id
            suffix = 2
            ...
            for extra in (kind, f"Priorität {priority}" if priority else ""):
            ...
            data = {
                ...
                "subtitle": "",
                "summary": summary,
                "planetaryBoundary": str(item.get("boundary", "")).strip(),
                "keywords": keywords,
                "imageId": "",
                "sourceId": "",
                "sourceTitle": str(item.get("sourceTitle", "")).strip(),
                "sourceUrl": source_url,
                "publicationDate": str(
                    item.get("publicationDate", today)
                ).strip() or today,
                "author": "",
                "editor": "",
                "created": today,
                ...
                "license": "",
                "notes": (
                    f"Importiert über Recherche-Assistent; "
        ...
        details = f"{imported} Meldung(en) wurden als Entwurf importiert."
        if skipped:
            details += "\n\nNicht importiert:\n- " + "\n- ".join(skipped[:10])
        messagebox.showinfo("Import abgeschlossen", details)
        self.status_var.set(f"{imported} Recherchemeldungen importiert.")
        self.tabs.select(self.article_tab)
    ...
    def _build_articles(self):
        ...
        image_box = ttk.LabelFrame(form, text="Titelbild", padding=10)
        image_box.pack(fill="x", pady=(12, 10))

        FieldRow(image_box, "Bild-ID", self.article_vars["imageId"])

        image_ai_row_1 = ttk.Frame(image_box)
        image_ai_row_1.pack(fill="x", pady=(6, 3))
        ttk.Button(
            image_ai_row_1,
            text="1. Bildprompt vorbereiten",
            command=self.open_image_workflow,
        ).pack(side="left")
        ttk.Button(
            image_ai_row_1,
            text="2. Bild übernehmen",
            command=self.import_article_image,
        ).pack(side="left", padx=(8, 0))

        image_ai_row_2 = ttk.Frame(image_box)
        image_ai_row_2.pack(fill="x", pady=(3, 0))
        ttk.Button(
            image_ai_row_2,
            text="Bild anzeigen",
            command=self.open_article_image,
        ).pack(side="left")
        self.image_ai_status = ttk.Label(
            image_ai_row_2,
            text="Noch kein Bild geprüft."
        )
        ...
        ttk.Label(
            image_box,
            text=(
                "Ablauf: Prompt kopieren → Bild in ChatGPT erzeugen → "
                "gespeicherte Bilddatei übernehmen."
            ),
            wraplength=720,
            justify="left",
