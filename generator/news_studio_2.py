...
class NewsStudio(tk.Tk):
    ...
    def _build_articles(self):
        ...
        self.article_list = tk.Listbox(left, exportselection=False)
        self.article_list.pack(fill="both", expand=True, pady=6)
        self.article_list.bind("<<ListboxSelect>>", self.load_selected_article)
        ...
        actionbar = ttk.Frame(form)
        actionbar.pack(fill="x", pady=12)
        ttk.Button(
            actionbar, text="Als Entwurf speichern",
            command=lambda: self.save_article("entwurf")
        ).pack(side="left")
        ttk.Button(
            actionbar, text="Speichern",
            command=self.save_article_current_status
        ).pack(side="left", padx=6)
        ttk.Button(
            actionbar, text="Freigeben und verschieben",
            command=lambda: self.save_article("freigegeben")
        ).pack(side="left")
        ttk.Button(
            actionbar, text="Datei löschen",
    ...
    def refresh_article_list(self):
        ...
        for label, folder in [("[Entwurf]", ENTWUERFE), ("[Artikel]", ARTIKEL)]:
            for path in json_files(folder):
                self.article_paths.append(path)
                self.article_list.insert("end", f"{label} {path.name}")
    ...
    def load_selected_article(self, _event=None):
        ...
        if not selection:
            return
        path = self.article_paths[selection[0]]
        try:
            data = read_json(path)
        except ValueError as exc:
    ...
    def save_article(self, status):
        ...
        self.refresh_all()
        messagebox.showinfo("Gespeichert", f"Artikel gespeichert:\n{rel(target)}")
    ...
    def save_source(self):
        ...
        self.refresh_all()
        messagebox.showinfo("Gespeichert", f"Quelle gespeichert:\n{rel(target)}")
    ...
    def run_generator(self):
        ...
        if success:
            messagebox.showinfo(
                "Generator",
                "news.json wurde erfolgreich erzeugt."
            )
        else:
            messagebox.showwarning(
                "Generator",
