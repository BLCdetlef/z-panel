...
class NewsStudio(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("ZUSTAND News Studio 2.1")
    ...
    def _build(self):
        ...
        header.pack(fill="x")
        ttk.Label(
            header, text="ZUSTAND News Studio 2.1",
    ...
    def _build_research(self):
        ...
        ttk.Label(
            outer,
            text=(
                "Links: seriöse Quellen öffnen. Rechts: eine gefundene Meldung kurz "
                "erfassen und direkt in den Meldungseditor übernehmen. "
                "Die automatische Webrecherche folgt später; diese Version funktioniert "
                "bereits ohne API-Schlüssel."
            ),
            wraplength=1000,
            justify="left",
    ...
    def start_research_assistant(self):
        ...
        messagebox.showinfo(
            ...
            (
                "ChatGPT wurde im Browser geöffnet und der Rechercheauftrag "
                "liegt in der Zwischenablage.\n\n"
                "Nächster Schritt:\n"
                "1. Im ChatGPT-Eingabefeld Strg+V drücken.\n"
                "2. Nachricht absenden.\n"
                "3. Geeignete Meldungen rechts in den Recherche-Assistenten übernehmen."
            ),
        )
        self.status_var.set("Recherche-Assistent gestartet; Prompt ist kopiert.")

    def _morning_research_prompt(self) -> str:
        source_names = ", ".join(name for _, name, _ in RESEARCH_SOURCES)
        return (
            "Recherchiere die wichtigsten neuen Meldungen seit gestern aus seriösen "
            ...
            "Schlage höchstens zehn Meldungen vor. Gib je Meldung aus: Priorität 1–5, "
            "Meldungsart, sachlicher Titel, Kurztext mit 350–550 Zeichen, betroffene "
            "planetare Grenze oder Querschnittsthema, Schlagwörter und direkten Link "
            "zur Originalquelle. Achte auf eine ausgewogene Mischung, darunter "
            "mindestens zwei positive oder konstruktive Meldungen, sofern seriös "
            "belegbare vorhanden sind. Prüfe besonders diese Quellen: "
            f"{source_names}."
        )
