...
class NewsStudio(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("ZUSTAND News Studio 5.1")
        ...
        if total_content == 0:
            messagebox.showwarning(
                ...
                (
                    ...
                    "Bitte prüfe, ob News Studio 5.1 im richtigen Projektordner liegt. "
    ...
    def _build(self):
        ...
        header.pack(fill="x")
        ttk.Label(
            header, text="ZUSTAND News Studio 5.1",
    ...
    def _build_articles(self):
        ...
        self.article_list = ttk.Treeview(
            left, columns=("status", "interview", "id", "title"), show="headings",
            selectmode="browse"
        )
        self.article_list.heading("status", text="Status")
        self.article_list.heading("interview", text="🎙 Interview")
        ...
        self.article_list.column("interview", width=90, anchor="center", stretch=False)
        ...
        self.article_vars = {
            ...
            "author": tk.StringVar(),
            "interviewPotential": tk.StringVar(),
        }
        ...
        interview_row = ttk.Frame(form)
        ttk.Label(interview_row, text="Intervieweignung", width=20).pack(side="left")
        ttk.Combobox(
            interview_row, textvariable=self.article_vars["interviewPotential"],
            values=("", "hoch", "mittel", "gering"), state="readonly"
        ).pack(side="left", fill="x", expand=True)
        interview_row.pack(fill="x", pady=3)
    ...
    def build_research_prompt(self, period: str) -> str:
        ...
        return (
            ...
            "eine JSON-Datei zum Import in ZUSTAND News Studio 5.1. Verwende exakt dieses Grundformat:\n\n"
            "{\n"
            ...
            "      \"priority\": 1,\n"
            "      \"interviewPotential\": \"hoch|mittel|gering\"\n"
            "    }\n"
            "  ]\n"
            "}\n\n"
            "Bevorzuge nach Möglichkeit Veröffentlichungen mit mindestens einer Autorin oder einem Autor "
            "einer deutschen Hochschule oder Forschungseinrichtung. Gib Institution und Land in den "
            "Schlagwörtern an und schätze die Eignung für ein etwa 20-minütiges Telefoninterview für "
            "die interessierte Öffentlichkeit als hoch, mittel oder gering ein.\n\n"
    ...
    def import_research_file(self):
        ...
        for index, raw in enumerate(items, start=1):
            ...
            interview_potential = str(raw.get("interviewPotential", "")).strip().lower()
            if interview_potential not in {"hoch", "mittel", "gering"}:
                interview_potential = ""
            notes_parts = ["Importiert über Recherche-Assistent 5.1"]
            if category:
            ...
            data = {
                ...
                "author": "",
                "interviewPotential": interview_potential,
                "created": today,
                "lastModified": today,
                "language": "de",
                "article": [],
        ...
        for folder, fallback in [(ENTWUERFE, "entwurf"), (ARTIKEL, "freigegeben")]:
            for path in json_files(folder):
                try:
                    ...
                    interview = str(data.get("interviewPotential", "")).strip().lower()
                except Exception:
                    status = "fehler"
                ...
                item_id = self.article_list.insert(
                    "", "end",
                    values=(status_labels.get(status, status), {"hoch": "⭐ hoch", "mittel": "◐ mittel", "gering": "· gering"}.get(interview, "–"), article_id, title)
                )
    ...
    def article_payload(self, forced_status=None):
        ...
        return {
            ...
            "interviewPotential": self.article_vars["interviewPotential"].get().strip(),
            "created": created,
            "lastModified": now,
            "language": "de",
            "article": [{
