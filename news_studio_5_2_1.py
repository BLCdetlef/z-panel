...
class NewsStudio52(base.NewsStudio):
    def __init__(self):
        ...
        self.title("ZUSTAND News Studio 5.2.1")
        self._replace_widget_text("ZUSTAND News Studio 5.1", "ZUSTAND News Studio 5.2.1")
        ...
        self.status_var.set(
            "News Studio 5.2.1 bereit │ Kontaktlayout verbessert"
        )
    ...
    def _add_contacts_tab(self) -> None:
        ...
        # Die Aktionsleiste bleibt immer sichtbar. Der übrige Platz wird zwischen
        # Kontaktdaten und verknüpften Meldungen aufgeteilt. Dadurch verschwindet
        # der untere Bereich auch auf kleineren Notebook-Bildschirmen nicht mehr.
        actionbar = ttk.Frame(right)
        actionbar.pack(side="bottom", fill="x", pady=(8, 0))
        ...
        contact_panes.pack(fill="both", expand=True)

        form = ttk.Frame(contact_panes, padding=(0, 0, 4, 4))
        association_box = ttk.LabelFrame(
            contact_panes, text="Verknüpfte Meldungen", padding=8
        )
        contact_panes.add(form, minsize=330, stretch="always")
        contact_panes.add(association_box, minsize=120, stretch="always")
        ...
        self.contact_notes = tk.Text(
            notes_frame,
            height=5,
            wrap="word",
            yscrollcommand=notes_scroll.set,
        )
        notes_scroll.configure(command=self.contact_notes.yview)
        notes_scroll.pack(side="right", fill="y")
        self.contact_notes.pack(side="left", fill="both", expand=True)

        links_scroll = ttk.Scrollbar(association_box, orient="vertical")
        ...
        links_scroll.configure(command=self.contact_links_text.yview)
        links_scroll.pack(side="right", fill="y")
        self.contact_links_text.pack(side="left", fill="both", expand=True)
