...
def inspect_release(write_output: bool = False):
    ...
    for path in json_files(ARTIKEL):
        ...
        sid = str(article.get("sourceId", "")).strip()
        source_title = str(article.get("sourceTitle", "")).strip()

        if not sid:
            if source_title:
                warnings.append(
                    f"{rel(path)}: sourceId fehlt; die vorhandene Quellenangabe "
                    f"'{source_title}' wird trotzdem ausgegeben."
                )
            else:
                warnings.append(
                    f"{rel(path)}: sourceId und sourceTitle fehlen."
                )
        elif source_ids and sid not in source_ids:
            warnings.append(
                f"{rel(path)}: Quelle '{sid}' wurde nicht als eigene Quelldatei gefunden."
            )
...
def fetch_remote_news(url: str):
    ...
    request = urllib.request.Request(
        request_url,
        headers={
            "User-Agent": "ZUSTAND-News-Studio/4.4",
...
class NewsStudio(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("ZUSTAND News Studio 4.4")
        ...
        if total_content == 0:
            messagebox.showwarning(
                ...
                (
                    ...
                    "Bitte prüfe, ob News Studio 4.4 im richtigen Projektordner liegt. "
    ...
    def _build(self):
        ...
        header.pack(fill="x")
        ttk.Label(
            header, text="ZUSTAND News Studio 4.4",
    ...
    def _build_overview(self):
        ...
        text = (
            ...
            "3. Bildprompt kopieren, Bild in ChatGPT erzeugen und übernehmen.\n"
            "4. Beitrag prüfen und in den Veröffentlichungsordner verschieben.\n"
            "5. Im Generator die fertige news.json erzeugen.\n"
            "6. Änderungen anschließend mit GitHub Desktop committen und pushen."
        )
