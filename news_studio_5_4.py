...
"""ZUSTAND News Studio 5.4 – produktiver Redaktions- und Interviewworkflow.
...
if not BASE_SCRIPT.exists():
    raise SystemExit(
        ...
        "Lege Version 5.4 in denselben Ordner wie Version 5.3."
    )
...
class NewsStudio54(base53.NewsStudio53):
    def __init__(self):
        super().__init__()
        self.title("ZUSTAND News Studio 5.4")
        self._replace_widget_text("ZUSTAND News Studio 5.3", "ZUSTAND News Studio 5.4")
        ...
        self.status_var.set(
            "News Studio 5.4 bereit │ produktiver Redaktions- und Interviewworkflow"
        )
    ...
    def _build_own_finds_tab(self) -> None:
        ...
        outer = ttk.Frame(self.own_finds_tab, padding=14)
    ...
    def copy_find_package(self) -> None:
        ...
        package = f"""Bitte prüfe dieses Fundstück für den ZUSTAND-Infoscreen und erstelle daraus einen strukturierten Studio-Datensatz.\n\nArt: {item.get('kind','')}\nArbeitstitel: {item.get('title','')}\nLink/DOI: {item.get('source','')}\nLokale Datei: {item.get('file','')}\n\nNotiz/Text:\n{item.get('text','')}\n\nBitte zuerst die Original- bzw. Primärquelle verifizieren. Danach: Kurztext (350–550 Zeichen), Kategorie, planetare Grenzen/Querschnitt, Priorität, Schlagwörter, Quellen, Bildidee, Unsicherheiten, Interviewpotenzial und geeignete Ansprechperson."""
    ...
    def _build_workflow_tab(self) -> None:
        ...
        outer = ttk.Frame(self.workflow_tab, padding=14); outer.pack(fill="both", expand=True)
        ...
        left = ttk.LabelFrame(body, text="Kontakte", padding=8); left.pack(side="left", fill="both", expand=True)
        self.workflow_tree = ttk.Treeview(left, columns=("name", "institution", "status", "links"), show="headings")
        ...
        right = ttk.LabelFrame(body, text="Bearbeitung", padding=10); right.pack(side="left", fill="y", padx=(10, 0))
    ...
    def refresh_workflow(self) -> None:
        ...
        if hasattr(self, "workflow_tree"):
            for item in self.workflow_tree.get_children(): self.workflow_tree.delete(item)
            for cid, contact in sorted(self.contact_db.get("contacts", {}).items(), key=lambda p: clean_text(p[1].get("name")).lower()):
                links = sum(1 for link in self.contact_db.get("articleLinks", {}).values() if cid in link.get("contactIds", []))
                wf = self.contact_db["workflow"].get(cid, {})
                self.workflow_tree.insert("", "end", iid=cid, values=(clean_text(contact.get("name")) or "Ohne Namen", clean_text(contact.get("institution")), wf.get("status", "Recherchiert"), links))
    ...
    def save_contact_status(self) -> None:
        ...
        wf["status"] = new
        if new != old: wf.setdefault("history", []).append({"at": now_iso(), "from": old, "to": new})
        self._save_db(); self.refresh_workflow(); self.status_var.set(f"Kontaktstatus: {new}")
    ...
    def _build_quality_tab(self) -> None:
        ...
        outer = ttk.Frame(self.quality_tab, padding=14); outer.pack(fill="both", expand=True)
